<html>
<head><title>
2002 Boston GNOME Summit Schedule</title>
<style>
td.nav  {
        height: 16px;
        }
td.cell {
	background-color: #eeeeee;
	text-align: center;
        }
td.white {
    background-color: #eeeeee;
	color: #eeeeee;
    text-align: top;
	width: 200px;
	height: 200px;
        }

td.title {
        background-color: #000000;
        color: #ffffff;
	text-align: center;
        height: 25px;
	font-weight: bold;
        }
td.bold {
        font-weight: bold;
	font-size: 11px;
	}

  </style>
</head>
<body text="#000000" bgcolor="#FFFFFF" link="#990000" vlink="#990000" alink="#990000">

  <center>
  <!-- Table for the black border. -->
  <table border="0" cellspacing="0" cellpadding="0" width="790">
    <tr><td bgcolor="#000000" colspan=3>
        <!-- Table for the content image. -->
        <table border="0" cellspacing="4" cellpadding="4" width="100%">
          <tr><td bgcolor="#ffffff" align="left">

    	  <!-- The image -->
    	  <img src="boston-summit.jpg" width=800 height=353 alt="2002 Boston GNOME Summit" border=0>

    	  <!-- The text -->
          <h1>Summit Schedule</h1>


<h2>Thursday, July 18th</h2>
<table cellpadding=5 cols=3 width="700" text="#ffffff" align=center bgcolor="#000000">
<tr bgcolor="#000000" align=center>
    <td width="16%" bgcolor="white"><font color="#ffffff"></font></td>
    <td width="16%"><font color="#ffffff">Room 33-319</font></td>
    <td width="16%"><font color="#ffffff">Room 33-418</font></td>
    <td width="16%"><font color="#ffffff">Room 33-419</font></td>
    <td width="16%"><font color="#ffffff">Room 1-135</font></td>
    <td width="16%"><font color="#ffffff">Room 1-136</font></td>
</tr>
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>9:00 AM</td>
    <td align=center colspan=5>Registration & Coffee (in room 10-250)</td> <!-- 33-419 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>10:00 AM</td>
    <td align=center colspan=5>Opening Address, Nat Friedman (in room 10-250)</td> <!-- 33-419 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>11:00 AM</td>
    <td rowspan=2 align=center> Hacking, etc.</td> <!-- 33-419 -->
    <td colspan=4 align=center>&quot;GNOME Release Process: 2.0 post-mortem and looking ahead,&quot; Jeff
      Waugh (in room 10-250)</td> <!-- 33-418 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>12:00 PM</td>
    <td valign=top>&quot;Extending Nautilus through Views and Scripts,&quot; Dave Camp</td> <!--  1-136 -->
    <td>&quot;Building applications with the GStreamer system,&quot; Thomas Vander Stichele</td> <!-- 33-422 -->
    <td align=center></td>
    <td align=center></td>
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>1:00 PM</td>
    <td colspan=5 align=center bgcolor="#fc87de">Lunch</td>
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>2:00 PM</td>
    <td rowspan=5 align=center> Hacking, etc.</td> <!-- 33-419 -->
    <td rowspan=2 valign=top>&quot;GNOME Project Infrastructure: cvs, www, bugzilla, etc.,&quot; Jeff Waugh.</td> <!--  1-135 -->
    <td rowspan=2>&quot;Setting your computer's time and other 'sysadmin' tasks,&quot; Chema Celorio</td> <!-- 33-418 -->
    <td rowspan=5></td> <!--  1-135 -->
    <td rowspan=5></td> <!--  1-136 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>3:00 PM</td>
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>4:00 PM</td>
    <td rowspan=3></td> <!-- 33-422 -->
    <td rowspan=2>&quot;Building a GNOME Office suite for users,&quot; Martin Sevior.</td>
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>5:00 PM</td>
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>6:00 PM</td>
    <td></td> <!-- 33-418 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>8:30 - 10:30 PM</td>
    <td colspan=5 align=center>Dinner at Fire + Ice (Leaving from Lobby 7 at 7:45)</td>
</table>

<h2>Friday, July 19th</h2>
<table cellpadding=5 cols=3 width="700" text="#ffffff" align=center bgcolor="#000000">
<tr bgcolor="#000000" align=center>
    <td width="16%" bgcolor="white"><font color="#ffffff"></font></td>
    <td width="16%"><font color="#ffffff">Room 33-319</font></td>
    <td width="16%"><font color="#ffffff">Room 33-418</font></td>
    <td width="16%"><font color="#ffffff">Room 33-419</font></td>
    <td width="16%"><font color="#ffffff">Room 1-135</font></td>
    <td width="16%"><font color="#ffffff">Room 1-136</font></td>
</tr>
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>10:00 AM</td>
    <td colspan=5 align=center>Coffee &amp; Donuts (in room 33-319)</td> <!-- 33-419 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>11:00 AM</td>
    <td rowspan=2> Hacking, etc.</td> <!-- 33-419 -->
    <td rowspan=2>&quot;Role-Playing Games for Fun and Profit: Usability Testing,&quot;
      Anna Dirks</td> <!--  1-135 -->
    <td rowspan=2>&quot;Building a GNOME Media Player,&quot; Thomas Vander Stichele</td> <!-- 33-418 -->
    <td rowspan=2>&quot;The New gnome.org Web Site,&quot; Jeff Waugh, Tim Ney, et al.</td> <!--  1-136 -->
    <td></td> <!--  1-136 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>12:00 PM</td>
    <td></td> <!--  1-136 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>1:00 PM</td>
    <td colspan=5 align=center bgcolor="#fc87de">Lunch</td>
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>2:00 PM</td>
    <td colspan=5 align=center>&quot;Plumbing and Other Utilities,&quot; Rob Pike (in room 10-250)</td> <!-- 33-418 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>3:00 PM</td>
    <td rowspan=4>Hacking, etc.</td> <!-- 33-418 -->
    <td rowspan=2>&quot;gphoto2: the foundation for a GNOME photo management application,&quot; Christophe Barbé</td>
    <td rowspan=2 valign=top>&quot;The File Selector,&quot; Havoc Pennington</td> <!-- 33-418 -->
    <td></td> <!-- 33-422 -->
    <td></td> <!--  1-135 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>4:00 PM</td>
    <td></td> <!--  1-135 -->
    <td></td> <!--  1-135 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>5:00 PM</td>
    <td></td> <!--  33-418 -->
    <td rowspan=2>&quot;GNOME Development and Release Process Discussion,&quot; Michael Meeks.</td> <!-- 33-422 -->
    <td></td> <!--  1-135 -->
    <td></td> <!--  1-136 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>6:00 PM</td>
    <td></td> <!-- 33-418 -->
    <td></td> <!--  1-135 -->
    <td></td> <!--  1-136 -->
</table>

<h2>Saturday, July 20th</h2>
<table cellpadding=5 cols=3 width="700" text="#ffffff" align=center bgcolor="#000000">
<tr bgcolor="#000000" align=center>
    <td width="16%" bgcolor="white"><font color="#ffffff"></font></td>
    <td width="16%"><font color="#ffffff">Room 33-319</font></td>
    <td width="16%"><font color="#ffffff">Room 33-418</font></td>
    <td width="16%"><font color="#ffffff">Room 33-419</font></td>
    <td width="16%"><font color="#ffffff">Room 1-135</font></td>
    <td width="16%"><font color="#ffffff">Room 1-136</font></td>
</tr>
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>10:00 AM</td>
    <td colspan=5 align=center>Coffee &amp; Donuts (in 33-319)</td> <!-- 33-419 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>11:00 AM</td>
    <td rowspan=2> Hacking, etc.</td> <!-- 33-419 -->
    <td>&quot;Demo of the X.Org Media Application Server,&quot; Leon Shiman</td> <!--  1-135 -->
    <td>&quot;Type and Toolkits - the road from Gutenburg and MacPaint,&quot; Owen Taylor</td> <!-- 33-418 -->
    <td></td> <!-- 33-422 -->
    <td></td> <!--  1-136 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>12:00 PM</td>
    <td>&quot;Bug tracking in GNOME,&quot; Luis Villa</td> <!--  1-136 -->
    <td>&quot;ABI Compatibility and ABICheck,&quot; Karl Runge</td> <!-- 33-418 -->
    <td></td> <!-- 33-422 -->
    <td></td> <!--  1-135 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>1:00 PM</td>
    <td colspan=5 align=center bgcolor="#fc87de">Lunch</td>
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>2:00 PM</td>
    <td rowspan=2> Hacking, etc.</td> <!-- 33-419 -->
    <td rowspan=2>&quot;Making Accessibility work for users,&quot; Bill Haneman.</td> <!-- 33-418 -->
    <td rowspan=2>&quot;Gaim: Instant Messaging for the Desktop User,&quot; Sean Egan</td> <!-- 33-422 -->
    <td rowspan=2>&quot;GU4DEC and GNOME event planning," Tim Ney et al.</td> <!--  1-135 -->
    <td></td> <!--  1-136 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>3:00 PM</td>
    <td></td> <!--  1-136 -->
<tr bgcolor="#c8d4e2">
    <td bgcolor="#acaba9" align=right>4:00 PM</td>
    <td colspan=5 align=center>Closing Address, Havoc Pennington (in room 6-120).</td> <!-- 33-419 -->
</table>
    	</td></tr>
        </table>
      </td></tr>
  </table>
</center>


</body>
</html>
 
