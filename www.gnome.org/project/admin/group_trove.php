...
while (list($catroot,$fullname) = each($CATROOTS)) 
{
	...
	// 'File Format' category is handled special
	if ($catroot == "275")
	{
		print "<br><select name=".$name." multiple size=10>\n";

		$res_cat = db_query('SELECT trove_cat_id,fullpath,parent FROM trove_cat WHERE '
			.'root_parent='.$catroot.' ORDER BY fullpath');

		while ($row_cat = db_fetch_array($res_cat)) 
		{
			// we don't want to show categories, only file formats
			if ($row_cat['parent'] != "275")
			{
				...
				print '>'.substr ($row_cat['fullpath'], 15)."\n";
			}
		}
