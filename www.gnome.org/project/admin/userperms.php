...
if ($submit) {
	...
	while ($row_dev = db_fetch_array($res_dev)) {
		...
		if (!$group->updateUser($row_dev['user_id'],$$admin_flags,$release_flags,$$member_role)) 
		{
...
$query_dev = "SELECT users.user_name AS user_name,"
...
if (!$res_dev || db_numrows($res_dev) < 1) {
	...
	<TR valign="bottom" BGCOLOR="'. $cur_color .'">
		<TD><b>'.$row_dev['user_name'].'</b></TD><TD>';

	echo member_role_box('role_user_'.$row_dev['user_id'],$row_dev['member_role']).'</TD>';

	print '<TD><INPUT TYPE="CHECKBOX" NAME="admin_user_'.$row_dev['user_id'].'" VALUE="A" ';
	print ((stristr($row_dev['admin_flags'],'A'))?'CHECKED':'').'> Admin &nbsp;&nbsp;';

	print '
		<INPUT TYPE="CHECKBOX" NAME="release_user_'.$row_dev['user_id'].'" VALUE="1" '.
		(($row_dev['release_flags']==1)?'CHECKED':'').'> Rel.Tech.</td>
